...
async function iniciar() {
    ...
    prodSnap.forEach(d => {
        ...
        if (objTf !== undefined) {
            if((typeof objTf === 'object' ? (objTf.visivel !== false) : true)) produtosGlobais.push({ ...item, precoFinal: typeof objTf === 'object' ? objTf.preco : objTf, catReal: (item.categoria || 'sorvete').toLowerCase().trim() });
        }
    });
    renderizarTabelas();
}
...
window.calcularTudo = () => {
    ...
    produtosGlobais.forEach((p, i) => {
        ...
        if ((cx * 10) % 5 !== 0) { alert(`⚠️ ERRO NO ENGRADADO: Apenas múltiplos de meio em meio.`); inputEng.value = ""; cx = 0; }
...
window.gerarExcelTransferencia = async () => {
    const razaoDestino = document.getElementById('cliRazao').value.trim(); const cnpjDestino = document.getElementById('cliCnpj').value.trim(); const cnpjOrigem = window.lojaCnpj || "CNPJ NÃO CADASTRADO";
    ...
    if(!lojaValida) { alert("⛔ OPERAÇÃO BLOQUEADA!\nSó é permitido transferir para lojas da rede devidamente cadastradas."); return; }
    ...
    try {
        ...
        const preencherAba = (nomeAba, categoriasPermitidas, tipoAba) => {
            ...
            } else if (tipoAba === 'ROMANEIO') { sheet.getCell('E7').value = cnpjOrigem; sheet.getCell('K7').value = razaoDestino.replace(/_/g, ' '); sheet.getCell('D8').value = window.resumoTransferencia.totalCaixas; sheet.getCell('G8').value = window.resumoTransferencia.totalPecas; sheet.getCell('L8').value = window.resumoTransferencia.valorTotal; }
    ...
    } catch (error) { console.error(error); alert("Ocorreu um erro ao processar a planilha."); }
