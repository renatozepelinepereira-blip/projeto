...
async function iniciar() {
    // Buscar filiais para o Autocomplete
    ...
    prodSnap.forEach(d => {
        ...
        produtosGlobais.push({ ...item, catReal: cat, precoFinal: 0 }); // Preço zero para transferência
        
        let imgHtml = item.imagem ? `<img src="${item.imagem}" class="img-produto" loading="lazy">` : `<div class="img-produto" style="display:flex;align-items:center;justify-content:center;background:#f1f5f9;">📦</div>`;
    ...
    Object.keys(htmlBuffers).forEach(k => {
        ...
        if(tbody) tbody.innerHTML = htmlBuffers[k] || `<tr><td colspan="6" style="text-align:center;">Nenhum produto.</td></tr>`;
    });
}
...
window.calcularTudo = () => {
    produtosGlobais.forEach((p, i) => {
        ...
        p.calcSubtotal = 0; // Sem valor na transf.
...
window.gerarExcelTransferencia = async () => {
    ...
    try { 
        // Adapta o processarExcelVenda para o formato de Transferência (valor R$ 0)
    ...
    finally { btn.innerHTML = "<span style='font-size: 20px;'>⬇️</span> GERAR TRANSFERÊNCIA EXCEL"; btn.disabled = false; }
};
