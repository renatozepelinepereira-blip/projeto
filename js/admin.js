...
window.toggleTabelaHistorico = () => {
    ...
    const isH = w.style.display === 'none';
    w.style.display = isH ? 'block' : 'none';
    b.innerText = isH ? '👁️ Esconder' : '👁️ Mostrar';
};

window.filtrarLogDash = () => {
    const t = document.getElementById('pesquisaLogAdmin').value.toLowerCase();
    document.querySelectorAll('.linha-hist-admin').forEach(i => {
        i.style.display = i.innerText.toLowerCase().includes(t) ? '' : 'none';
    });
};
...
async function carregarDashboard() {
    ...
    try {
        ...
        document.getElementById('dashLojas').innerText = sL.size - 1;
        document.getElementById('dashProdutos').innerText = sP.size;
            ...
            if(ok) { 
                ...
                html += `<tr class="linha-hist-admin">
                    ...
                    <td style="display: flex; gap: 8px; justify-content: center;">
                        <button class="btn-small" style="background:#f1f5f9; color:#0f172a; border: 1px solid #cbd5e1;" title="Visualizar" onclick="window.visualizarLog('${d.id}')">👁️</button>
                        <button class="btn-small" style="background:#10b981; color:white;" title="Baixar Excel" onclick="window.regerar('${d.id}')">⬇️</button>
                    </td>
                </tr>`; 
            }
        });
        document.getElementById('corpoTabelaHistorico').innerHTML = html;
...
window.visualizarLog = (id) => {
    ...
    let html = `<div style="margin-bottom: 15px;">
        <b>Data:</b> ${log.dataHora?.toDate ? log.dataHora.toDate().toLocaleString('pt-BR') : 'N/A'}<br>
        <b>Loja:</b> ${log.nomeLoja || log.lojaId}<br>
        <b>Ação:</b> ${log.acao}<br>
        <b>Destino/Cliente:</b> ${log.destino || log.dadosPlanilha?.razao || '-'}
    </div>`;

    if(log.dadosPlanilha && log.dadosPlanilha.itens && log.dadosPlanilha.itens.length > 0) {
        html += `<table style="width:100%; border:1px solid #e2e8f0; border-collapse:collapse;">
            <thead style="background:#e2e8f0;">
                <tr>
                    <th style="padding:8px; text-align:left; font-size:12px;">Cód</th>
                    <th style="padding:8px; text-align:left; font-size:12px;">Descrição</th>
            ...
            <tbody>`;
        log.dadosPlanilha.itens.forEach(i => {
            html += `<tr style="border-bottom:1px solid #e2e8f0;">
                <td style="padding:8px; font-size:13px;">${i.codigo}</td>
                <td style="padding:8px; font-size:13px;">${i.descricao}</td>
                <td style="padding:8px; text-align:center; font-size:13px;">${i.calcTotalUnidades}</td>
                <td style="padding:8px; text-align:right; font-size:13px; color:var(--primary); font-weight:600;">R$ ${(i.calcSubtotal||0).toFixed(2)}</td>
            </tr>`;
        });
        html += `</tbody></table>`;
        if(log.dadosPlanilha.totalV) {
            html += `<div style="text-align:right; margin-top:15px; font-size:18px;">Total: <b style="color:var(--primary);">R$ ${log.dadosPlanilha.totalV.toFixed(2)}</b></div>`;
        }
    } else {
        html += `<div style="padding:15px; background:#f1f5f9; text-align:center; border-radius:8px;">Nenhum item detalhado salvo nesta operação.</div>`;
    }
    
    document.getElementById('conteudoDetalhesLog').innerHTML = html;
    document.getElementById('btnRegerarPlanilhaModal').onclick = () => window.regerar(id);
...
async function carregarProdutos() {
    ...
    snap.forEach(d => {
        ...
        html += `<tr>
            ...
            <td>${p.categoria}</td>
            <td style="display: flex; gap: 8px; justify-content: center;">
                <button class="btn-small" style="background:#3b82f6; color:white;" title="Editar" onclick="window.abrirEdicaoProduto('${p.codigo}')">✏️</button>
                <button class="btn-small" style="background:#ef4444; color:white;" title="Excluir" onclick="window.excluirProduto('${p.codigo}')">🗑️</button>
            </td>
        </tr>`;
    });
...
window.excluirProduto = async (cod) => {
    if(confirm(`ATENÇÃO: Tem certeza que deseja excluir permanentemente o produto ${cod}?`)) {
        try {
            await deleteDoc(doc(db, "produtos", cod));
...
async function carregarTabelasPrecos() {
    ...
    sL.forEach(u => {
        ...
        cont.innerHTML += `<div class="loja-check-item" style="display:flex; gap:10px; padding:8px; border-bottom:1px solid #f1f5f9; align-items: center;">
            <input type="checkbox" class="chk-loja" value="${u.id}" data-search="${txt}" style="width:18px; height:18px; margin:0;">
            <label style="font-size:13px; margin:0; cursor:pointer;"><b>${fil?`Filial ${fil}`:''}</b> ${d.nomeLoja || u.id}</label>
        </div>`;
    });
}
...
window.vincularTabelaEmMassa = async () => {
    ...
    if(!tab || !ids.length) return alert("Selecione a tabela e as lojas!");
    ...
    try {
        ...
        alert("Vínculo aplicado com sucesso!"); carregarLojas();
    } finally { btn.innerText = "💾 Aplicar Tabela"; }
};
...
async function carregarLojas() {
    ...
    snap.forEach(d => {
        ...
        html += `<tr><td>${d.id}</td><td>${u.nomeLoja || '-'}</td><td>${u.cnpj || '-'}</td><td><b>${(u.tabelaPreco || '-').toUpperCase()}</b></td><td style="text-align: center;"><button class="btn-small" style="background:#3b82f6; color:white;" onclick="window.abrirEdicaoLoja('${u.id}')">✏️</button></td></tr>`;
    });
